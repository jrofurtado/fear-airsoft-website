...
typedef void OnDataHandler(var jsonObject);
...
class JsonpCallback {
  ...
  Future doCallback(String url, [OnDataHandler onData]) {
    ...
    subscription = window.onMessage.listen(_onMessage);

    //add the correct text back into the JavaScript handler
    _addScriptText();
    document.body.append(_script); //add the jsonp callback javascript
    ...
    script.remove(); //remove the script which initiates the call
